...
\newtheorem{TheoremLike}{\text{\bfseries{\emph{BAD-EXPANSION}}}}
...
\begin{abstract}
  Presentiamo qui alcuni cenni di teoria riguardanti la nostra
  implementazione didattica del crittosistema RSA.  Nel seguito si assume
  una conoscenza di base dell'aritmetica teorica --- i.e., principio di
  induzione e del minimo intero, definizione e semplici propriet\`a dei
  numeri primi e composti, \TFA, divisione con resto, eccetera.  Alcuni
  di questi concetti saranno comunque brevemente richiamati.
\end{abstract}
...
facilmente accessibili nel seguito.
...
< \abs{b}$. Questi due interi sono unici, e sono chiamati rispettivamente
...
offrire dimostrazioni rimarchevolmente semplici di questi teoremi, senza
assumere il \TFA.
%
...
Sia $d$ un intero che divide sia $m$ che $ab$.  Allora $d$ ed $a$
sono coprimi (ogni loro divisore comune $> 1$ dividerebbe sia $m$
che $a$, assurdo), e quindi, essendo $d \divides ab$, deriva dal
...
segue $m \divides \p{a -b}$, ossia $a \congruent b \mmod m$.
\end{proof}
...
\begin{eqnarray}
  a^{2n}   & \congruent & \p{\rem{a^2}{m}}^n \mmod m
    \label{eq:sq-mul-even}
  \\
  a^{2n+1} & \congruent & a \p{\rem{a^2}{m}}^n \mmod m
    \label{eq:sq-mul-odd}
\end{eqnarray}
%
Eccolo in dettaglio:
...
$k$, risulta $e_k = 1$ e:
\begin{equation}\label{eq:last-sq-mul-pass}
  a^m \congruent b_k f_k \mmod m
\end{equation}
Inoltre, $k \leq \log_2 n$, cosicch\`e l'algorimo termina dopo al
pi\`u\, $\floor{\log_2 n}$ passi.
\end{algorithm}
...
\mmod m$, da cui la~\eqref{eq:last-sq-mul-pass} ponendo $i = k$.  Inoltre,
...
y \mmod m$. Dunque l'insieme $\setdef{ax}{0 \leq x < p}$ \`e un
sistema completo di residui modulo $m$, cosicch\`e
da~\eqref{eq:product-complete-residue-system} deriva:
\begin{displaymath}
  \prod_{1 \leq x < p} ax \congruent \prod_{1 \leq x < p} a \mmod m
\end{displaymath}
ossia:
\begin{displaymath}
...
Alice, il destinatario \`e Bob, i pacchi fanno le veci dei messaggi,
i lucchetti recitano la parte delle chiavi pubbliche, e le chiavi dei
lucchetti recitano la parte delle chiavi private.

Bob spedisce ad Alice il suo lucchetto, gi\`a aperto, ma custodisce
gelosamente la chiave dello stesso. Alice riceve il lucchetto e, con
esso, chiude il pacco e lo spedisce a Bob. Bob riceve il pacco e pu\`o
aprirlo con la chiave di cui \`e l'unico proprietario. Similamente, se
adesso Bob volesse mandare un altro pacco ad Alice, dovrebbe farlo
...
chiave pubblica a partire dalla dalla chiava privata \`e facile ed
...
implementazioni ``ragionevolmente efficienti'' di RSA. Tali implementazioni
si avvantaggiano dell'uso dell'algoritmo~\eqref{alg:square-and-multiply}
...
La sicurezza del crittosistema RSA dipende quindi dall'impossibilit\`a
...
Sia $n = pq$ prodotto di due primi distinti, e siano $m$, $e$ due
interi con $e > 0$.  Siano poi \,$e_p := \rem{e}{(p-1)}$,
\,$e_q := \rem{e}{(q-1)}$, e sia $x$ intero tale che
$px \congruent 1 \mmod q$. Risulta allora:
\begin{displaymath}
  m^e \congruent m^{e_p} + px\,\big(m^{e_q} - m^{e_p}\big)\mmod n
\end{displaymath}
\end{theorem}
%
\begin{proof}
Poniamo $a = m^{e_p} + px\p{m^{e_q} - m^{e_p}}$.  In virt\`u del
teorema~\eqref{thm:TFA-mul-2}, per provare la tesi dobbiamo solo
dimostrare che $m^e \congruent a \mmod p$ e $m^e \congruent a \mmod q$.
Ma \`e immediato verificare che:
\begin{displaymath}
  a \congruent m^{e_p} \mmod p
    \text{\quad e \quad}
  a \congruent m^{e_q} \mmod q
\end{displaymath}
e dunque ci basta dimostrare che $m^e \congruent m^{e_p} \mmod p$ e
$m^e \congruent m^{e_q} \mmod q$. Se $p \divides m$ si ha banalmente:
\begin{displaymath}
  m^e \congruent m^{e_p} \congruent 0 \mmod p
\end{displaymath}
mentre se $p \notdivides m$, posto $e = (p - 1)t_p + e_p$ con $t_p$ intero
opportuno, si ha per il teorema di Fermat (applicato nel secondo
passaggio):
\begin{displaymath}
  m^e = \p{m^{p-1}}^{t_p} \cdot m^{e_p} \congruent 1^{t_p} \cdot m^{e_p}
      \congruent m^{e_p} \mmod m
\end{displaymath}
coicch\`e in ogni caso $m^e \congruent m^{e_p} \mmod p$. Analogamente si
dimostra che $m^e \congruent m^{e_q} \mmod q$, e la tesi \`e provata.
\end{proof}
...
a riportarlo.}. Si noti che i parametri qui utilizzati sono
...
\end{eqnarray*}
e quindi $d = \rem{\p{-367}}{3120} = 3120 - 367 = 2753$.
...
operando modulo $61$ abbiamo:
{
  ...
  2790^{2753}
    & \congruent &
      {45}^{53} \congruent \p{{45}^2}^{26} \cdot 45 = 1764^{26} \cdot 45
      \congruent 12^{26} \cdot 45 = 144^{13} \cdot 45 \congruent
    \nonumber\\
    & \congruent &
...
L'algoritmo RSA ``semplice'', come descritto nella precedente sezione%
...
\item
  ...
  testo in chiaro viene sempre trasformato nello stesso testo criptato.
  ...
  chiaro che egli aveva ``tirato a indovinare'' \`e quello corretto.
\item
  ...
  differente, ed apparentemente innocuo, testo cifrato $c' = \rem{cr^e}{n}$
...
\begin{itemize}
 ...
 \item $G$ una funzione di hash crittografica che espande stringhe di bit
       di lunghezza $k_0$ in stringhe di bit di lunghezza $n - k_0$;
 \item $H$ una funzione di hash crittografica che espande stringhe di bit
       di lunghezza $n - k_0$ in stringhe di bit di lunghezza $k_0$;
\end{itemize}
%
Si noti che in genere le funzioni $H$ e $G$ (che non possono essere
fissate a priori, in quanto dipendono dalla dimensione della chiave)
sono in genere derivate in maniera appropriata da funzioni di hash
crittografiche standard.
...
e deterministiche di RSA inutilizzabili in un eventuale attacco.

\begin{remark}
La nostra implementazione di RSA in python non offre attualmente alcuna
funzionalit\`a di padding, ed \`e quindi totalmente inappropriata per
ogni uso pratico.  Tuttavia, essendo tale implementazione deliberatamente
progettata per essere estesa in modo semplice e naturale, non dovrebbe
essere difficile adattarla per introdurvi schemi di padding.
\end{remark}
...
di Fermat}, un algoritmo piuttosto elementare che si pu\`o applicare a
tutti gli interi dispari, e che risulta particolarmente efficace per
numeri aventi fattori non banali ``vicini'' tra loro.
...
\begin{eqnarray*}
  ...
  B_{i+1} & := & B_i + 2 a_i + 1
\end{eqnarray*}
Dopo l'ultima iterazione $k$, si ha che $a := a_k$ e $b := \sqrt{B_k}$
sono interi, che $a - b$ \`e un fattore non banale di $n$, e che
$n = (a - b)(a + b)$.
\end{algorithm}
...
$t$ intero tale che $p - q \leq 2\sqrt{2t}\sqrt[4]{n}$. Allora il metodo
di Fermat applicato ad $n$ termina in al pi\`u $t$ passi.
\end{theorem}
\begin{proof}
Poich\`e il metodo di Fermat offre sempre (quando applicato ad interi
composti) una fattorizzazione non banale, poich\`e $n$ ammette un'unica
fattorizzazione non banale, e poich\`e, $p > q$, avremo che $p = a + b$
e $q = a - b$, dove $a$ e $b$ sono ricavati dal metodo di Fermat.

\medskip\noindent
Abbiamo poi:
\begin{displaymath}
  2b = p - q \leq 2\sqrt{2t}\sqrt[4]{n} \implies b^2 \leq 2t\sqrt{n}
\end{displaymath}
da cui:
\begin{displaymath}
...
Noti $n$ ed $e$ tali che $ed \congruent 1 \mmod{\ephi{n}}$, esiste un
