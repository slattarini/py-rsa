...
qualunque (anche non coprimo con $m$), allora risulta:
  $$ a^{k\,\phi(m) + 1} \congruent a \mmod m $$
per ogni intero $k$.
\end{theorem}
%
\begin{proof}
...
$a^{k\phi(m)} = a^{k(p-1)(q-1)} = {\p{a^{p-1}}}^{k(q-1)} \congruent
1^{k(q-1)} \congruent 1 \mmod p$.  D'altra parte, se $p \divides a$,
allora $a^{k\phi(m) + 1} \congruent 0^{k\phi(m) + 1} = 0 \congruent a
\mmod p$. In ogni caso, $a^{k\phi(m) + 1} \congruent a \mmod p$.
Similamente, $a^{k\phi(m) + 1} \congruent a \mmod q$, e quindi, essendo
$p$ e $q$ coprimi (in quanto primi distinti), $a^{k\phi(m) + 1}
\congruent a \mmod{pq}$,
...
$\phi(n) = (p-1)(q-1)$, e sia $d$ il suo inverso modulo $\phi(n)$.  Allora
$m^{ed} \congruent m \mmod n$ per ogni intero $m$.
\end{corollary}
