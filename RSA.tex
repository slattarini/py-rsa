...
$a^{\phi(m) + 1} \congruent a \mmod m$ per \emph{ogni} intero $a$
(anche non coprimo con $m$).
\end{theorem}
%
\begin{proof}
...
$a^{\phi(m) + 1} \congruent 0^{\phi(m) + 1} = 0 \congruent a \mmod p$.
In ogni caso, $a^{\phi(m) + 1} \congruent a \mmod p$.  Similamente,
$a^{\phi(m) + 1} \congruent a \mmod q$, e quindi, essendo $p$ e $q$ coprimi
(in quanto primi distinti), $a^{\phi(m) + 1} \congruent a \mmod{pq}$,
