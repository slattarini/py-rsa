...
rocedimento, poich\`e essi non sono stati implementati nel nostro modulo
pythone.}:
...
La chiave pubblica consiste della coppia $(n,\,e)$, e la chiave
privata consiste della terna $(p,\,q,\,e)$.

\TODO: immediato ricavare pubblica da privata; l'opposto deve essere
infattibile in pratica!

\subsection{RSA per messaggi binari qualunque}

Dalla precedente sezione deriva chiaramente che l'algoritmo RSA
``semplice'' \`e in grado di operare solo su numer interi limitati,
ossia i possibili ``messaggi'' sono tutti e soli gli interi $m$ tali che
$0 \leq m < n$ (la loro massima grandezza dipende quindi dal modulo $n$
della chiave).

Tuttavia, nella pratica, si \`e generalemnte interessati a criptare
generiche sequenze di bytes\footnote{Con le quali, nei moderni sistemi
informativi, tutto \`e rappresentabile.}.  Il metedo pi\`u semplice per
far ci\`o consiste nel complementare l'algoritmo RSA con protocolli e
algoritmi in grado di convertire efficientemente input ed output binari
\emph{da} ed \emph{in} sequenze di interi di grandezza massima prestabilita.

Descriviamo di seguito i semplici algoritmi di conversioni di tale tipo
utilizzati nella nostra implementazione di RSA.

\begin{algorithm}[Conversioni tra plaintext e sequenze di interi]
\TODO
\end{algorithm}

\begin{algorithm}[Conversioni tra ciphertext e sequenze di interi]
\TODO
\end{algorithm}
